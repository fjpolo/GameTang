...
# GAMETang SD Card Image Format

nand2mario, 2023.3

The gametank2img.py script packs multiple [igametank](https://www.gametankdev.org/wiki/IGAMETANK) rom images into 
...
structure. The format is designed for easy parsing and processing by hardware, as GAMETang aims
...
```
  ...
  |  |  |      ...       |
  |  |  +----------------+
  |  +->| GAMETANK image 0    |  variable # of sectors
  |  |  +----------------+
  |  `->| GAMETANK image 1    |
  |     +----------------+
  |     |      ...       |
...
Each "meta sector" contains meta data about up to 20 igametank roms. It is followed by a corresponding
...
* 16B (bytes) header
    ...
    * 1B background_color, in GAMETANK color (0 to 63)
    * 1B foreground_color, in GAMETANK color (0 to 63)
    * 3B not used
...
* This is a simple full screen (256x240) bitmap of the menu screen in GAMETANK color, one byte per pixel.
  Since each sector is 512 bytes, holding 2 ligametank of pixels, therefore the whole image takes in total
  120 sectors (60KB).

Format of **GAMETANK image**:
* These are just raw igametank roms stored in whole 512-byte sectors.
